...
async function resolveZurgToRdLink(zurgFilePath, rdApiKey) {
  try {
    ...
    const matchingTorrent = torrentsRes.data.find(t => {
      ...
      // Fallback: normalize and partial match
      ...
      return normOriginal === normPack ||
             normOriginal.includes(normPack) ||
             normPack.includes(normOriginal) ||
             normFilename.includes(normPack) ||
             normPack.includes(normFilename);
    });
